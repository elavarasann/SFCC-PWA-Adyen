...
const AdyenPaymentComponent = ({amount, currency = 'USD', onPaymentCompleted, onPaymentFailed}) => {
        ...
        const initializeCheckout = async () => {
            try {
                ...
                if (!configResponse.ok || !sessionResponse.ok) {
                    const response = !configResponse.ok ? configResponse : sessionResponse
                    const body = await response.json().catch(() => ({}))
                    throw new Error(body.message || 'Could not initialize Adyen checkout.')
                }
                ...
                const checkout = await AdyenCheckout({
                    ...
                    onError: (checkoutError) => {
                        ...
                        setError(checkoutError.message || 'The payment could not be completed.')
                ...
                if (isActive) {
                    setError(checkoutError.message || 'Could not initialize Adyen checkout.')
                    setLoading(false)
                }
            }
        }
    ...
    }, [currency, hasValidAmount, onPaymentCompleted, onPaymentFailed, paymentAmount])

    if (!hasValidAmount) {
        return <div role="alert">Add an item to the cart before starting an Adyen payment.</div>
    }
        ...
        <div className="adyen-pwa-dropin">
            ...
            <button
                ...
                style={{
                    ...
                    borderRadius: '4px',
                    color: '#fff',
                    cursor: loading || !isReadyToPay || isSubmitting ? 'not-allowed' : 'pointer',
                    fontSize: '16px',
                    fontWeight: 600,
                    marginTop: '16px',
                    minHeight: '44px',
