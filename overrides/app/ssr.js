...
import { checkoutAPI, MERCHANT_ACCOUNT } from "./api/adyen-client";
...
const ADYEN_ENVIRONMENT = (
  process.env.ADYEN_ENVIRONMENT || "TEST"
).toUpperCase();

function adyenConfigurationIsValid() {
  return Boolean(
...
function getValidatedAdyenAmount(amount) {
...
async function sendMagicLinkEmail(
  ...
  if (
    landingPath === config.app.login?.passwordless?.landingPath &&
    redirectUrl
  ) {
...
export const createRemoteJWKSet = (tenantId) => {
  ...
  const configTenantId =
    appConfig.commerceAPI.parameters.organizationId.replace(/^f_ecom_/, "");
...
export async function jwksCaching(req, res, options) {
  ...
  const isValidRequest =
    tenantIdRegExp.test(tenantId) && shortCodeRegExp.test(shortCode);
  if (!isValidRequest)
    return res
      .status(400)
      .json({
  ...
  try {
    ...
    res
      .status(400)
      .json({ error: `Error while fetching data: ${error.message}` });
  }
}
...
const { handler } = runtime.createHandler(options, (app) => {
  ...
  // The client key is safe to expose. The API key remains server-side at all times.
  app.get("/api/adyen/config", (req, res) => {
    if (!adyenConfigurationIsValid()) {
      return res.status(503).json({
        message:
          "Adyen test credentials are not configured. Check the local .env file.",
      });
    }
  ...
  // Local proof of concept only: a real implementation must derive the amount and order
  // reference from the trusted B2C Commerce basket/order, never from the browser request.
  app.post("/api/adyen/sessions", async (req, res) => {
    ...
    const amount = getValidatedAdyenAmount(req.body.amount);
    if (!amount) {
      return res
        .status(400)
    ...
    try {
      ...
      const merchantReference = `local-pwa-${Date.now()}`;
      ...
      return res
        .status(502)
        .json({ message: "Adyen could not create a test payment session." });
    }
  });

  // 2. Handle additional payment details (redirects, 3DS challenge)
  app.post("/api/adyen/payments/details", async (req, res) => {
    try {
      const response = await checkoutAPI.PaymentsApi.paymentsDetails(req.body);
      res.json(response);
    } catch (err) {
      console.error("Adyen payment details error", err);
      res.status(500).json({ message: err.message });
    }
  });

  // 3. Webhook receiver (Adyen -> your server, notifies async payment result)
  app.post("/api/adyen/webhook", async (req, res) => {
    try {
      if (!process.env.ADYEN_HMAC_KEY) {
        return res.status(503).send("[not accepted]");
      }
      ...
      for (const { NotificationRequestItem: item } of items) {
        ...
        if (!isValid) {
          console.warn("Invalid Adyen webhook signature", item.pspReference);
          return res.status(401).send("[not accepted]");
        }

        console.log(
          `Webhook: ${item.eventCode} success=${item.success} ref=${item.merchantReference}`
        );
        // TODO: update your local "order" record here
      }
      res.send("[accepted]");
    } catch (err) {
      console.error("Webhook error", err);
      res.status(500).send("[not accepted got it]");
    }
  });
