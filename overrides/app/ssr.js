...
const ADYEN_ENVIRONMENT = (process.env.ADYEN_ENVIRONMENT || 'TEST').toUpperCase()
...
const {handler} = runtime.createHandler(options, (app) => {
    ...
    app.get('/api/adyen/config', (req, res) => {
        if (!adyenConfigurationIsValid()) {
            return res.status(503).json({
                message: 'Adyen test credentials are not configured. Check the local .env file.'
            })
        }

        return res.json({clientKey: process.env.ADYEN_CLIENT_KEY, environment: 'test'})
    })
    ...
    app.post('/api/adyen/sessions', async (req, res) => {
        ...
        try {
            ...
            return res.status(502).json({message: 'Adyen could not create a test payment session.'})
        }
    })
    ...
    app.post('/api/adyen/webhook', async (req, res) => {
        try {
            ...
            for (const {NotificationRequestItem: item} of items) {
                const isValid = validator.validateHMAC(item, process.env.ADYEN_HMAC_KEY)
                if (!isValid) {
